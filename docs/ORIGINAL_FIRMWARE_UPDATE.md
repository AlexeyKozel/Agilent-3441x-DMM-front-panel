...
Both slices are byte-identical to the included `firmware/34410A_front_panel_firmware.bin`. Metadata at `0x1000..0x100D` is `00 09 60 00 1E 01 00 00 00 00 00 00 00 00`; revision at `0x1000..0x1001` is `0x0009`.
...
`0xFF03` is an entry into the MCU's internal ROM ISP service. The application image ends below that address, so internal ROM implementation bytes must not be attributed to the included file. The PPC APP contains enough evidence to close the wire protocol, responses, and full stock host flow without those ROM bytes.
...
For writes, the panel returns an exact echo followed by `.
`. For reads, two ASCII hex digits containing the read byte occur after the echo and before `.
`.
...
6. clear a security byte and erase seven sectors at `0x0000,0x0400,...,0x1800`;
7. verify boot vector/status/UCFG again;
...
10. read final boot vector/status/UCFG;
11. `stopProgramming()` and restore runtime UART operation.

Any record-generation, communication, response, or verification error aborts the chain.
